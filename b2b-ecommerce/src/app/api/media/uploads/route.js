  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        // Get image dimensions for images (basic approach)
        let metadata = {};
        if (fileType === 'image') {
          // You can use sharp or other libraries here for better image processing
          // For now, we'll leave metadata empty and let the frontend handle it
          metadata = {
            width: null,
            height: null,
            aspectRatio: null
    ...
    return NextResponse.json({
      ...
      status: successfulUploads.length > 0 ? 201 : 400 
    });
